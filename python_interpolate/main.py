...
class Framerate_upscale:
    ...
    def mean_upscaling(self, upscaled_dir: str) -> None:
        ...
        new_file = iio.get_writer(
            upscaled_dir, format='FFMPEG', mode='I', fps=int(self.fps*2))
        ...
        for image in self.vid.iter_data():
            if i:
                ...
                frame_between = np.zeros(
                    (self.width, self.height, 3, 2), np.uint8)
    ...
    def cubic_interp_upscaling(self, upscaled_dir: str) -> None:
        ...
        for y in range(self.width):
            for x in range(self.height):
                for z in range(3):
                    pixel_frame_color = [data[i][y][x][z] for i in range(
                        len(data))]  # take nembers for color in pixel
    ...
    def mean_quadruple_upscaling(self, upscaled_dir: str) -> None:
        ...
        new_file = iio.get_writer(
            upscaled_dir, format='FFMPEG', mode='I', fps=int(self.fps*4))
        ...
        for image in self.vid.iter_data():
            if i:
                ...
                frame_between2 = np.zeros((self.width, self.height, 3, 2), np.uint8)
                frame_between2[:, :, :, 0] = frame_between
                frame_between2[:, :, :, 1] = image
                frame_between2 = frame_between2.mean(axis=3)
                new_file.append_data(frame_between2)

                frame_between3 = np.zeros((self.width, self.height, 3, 2), np.uint8)
